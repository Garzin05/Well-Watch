...
class _DiabetesPageState extends State<DiabetesPage> {
  ...
  Widget build(BuildContext context) {
    ...
    for (var i = 0; i < last7.length; i++) {
      final r = last7[i];
      if (r.glucoseMgDl != null) glucose.add(FlSpot(i.toDouble(), r.glucoseMgDl!.toDouble()));
    }

    return Scaffold(
      appBar: AppBar(title: const Text('Diabetes')),
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () => _addGlucose(p),
          ...
          Card(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Row(children: [
                    Expanded(child: Text('Glicose — últimos 7 dias', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w600, color: scheme.primary))),
                    _Legend(labels: const ['Glicose'], colors: [scheme.primary]),
                  ]),
                  ...
                  SizedBox(
                    ...
                    child: LineChart(
                      LineChartData(
                        ...
                        lineBarsData: [
                          LineChartBarData(isCurved: true, color: scheme.primary, barWidth: 3, dotData: const FlDotData(show: false), spots: glucose),
                        ],
                      ),
                    ),
                  ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  ...[...p.records].reversed.take(12).map((r) {
                    ...
                    return ListTile(leading: const Icon(Icons.bloodtype_rounded), title: Text(g), subtitle: Text(when));
                  }),
                ],
              ),
            ),
        ...
        return Padding(
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                Row(children: [
                  Expanded(child: Text('Adicionar glicose — ${p.name}', style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 18))),
                ...
                Align(
                  ...
                  child: FilledButton.icon(
                    onPressed: () {
                      final g = double.tryParse(gCtrl.text.replaceAll(',', '.'));
                      if (g == null) {
                        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Informe a glicose.')));
                        return;
                      }
                      appData.addVital(patientId: p.id, date: when, glucoseMgDl: g);
                      Navigator.pop(context);
                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Glicose adicionada.')));
                      setState(() {});
                    },
